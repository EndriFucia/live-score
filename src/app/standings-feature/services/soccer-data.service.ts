...
export class SoccerDataService {
    ...
    leaugeSelectedSubject = new BehaviorSubject<string>('England');
    leagueSelectedAction$ = this.leaugeSelectedSubject.asObservable();

    leauges$ = this.http
        .get<ResponseBaseModel<LeaugeDetails>>(
            `${environment.apiBaseUrl}/leagues?&season=${new Date().getFullYear()}`,
            { headers: this.headers }
        )
        .pipe(tap((data) => console.log(data), shareReplay(1)));

    selectedLeague$ = combineLatest([this.leauges$, this.leagueSelectedAction$]).pipe(
        map(([leaguesResponse, country]) =>
            leaguesResponse.response.find(
                (league) => league.country.name == country && league.league.name == this.countryLeauges[country]
            )
        ),
        shareReplay(1)
    );
